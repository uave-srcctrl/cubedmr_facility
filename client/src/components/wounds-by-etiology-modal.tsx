...
interface WoundsByEtiologyModalProps {
...
export function WoundsByEtiologyModal({
  patients,
  totalWounds,
  totalPatients,
  etiology,
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-4 border-b">
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-full", getEtiologyColor(etiology).replace('text-', 'bg-').replace('600', '100'))}>
              <CircleDot className={cn("h-6 w-6", getEtiologyColor(etiology))} />
            </div>
            <div>
              <DialogTitle className="text-2xl font-bold">
                {normalizeEtiology(etiology)} Wounds
              </DialogTitle>
              <DialogDescription className="mt-1">
                Wounds of type: {normalizeEtiology(etiology)}
              </DialogDescription>
            </div>
          </div>
          {!isLoading && (
            <div className="flex gap-4 mt-4">
              ...
              <Badge className={cn("text-sm px-3 py-1", getEtiologyColor(etiology))}>
        ...
        <div className="space-y-4 pt-4">
          {isLoading ? (
            <EcgLoader title={`Loading ${normalizeEtiology(etiology)} wounds...`} minHeight="min-h-[200px]" />
          ) : patients.length === 0 ? (
            <Card>
              <CardContent className="pt-6">
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-lg font-medium text-gray-600">
                    No {normalizeEtiology(etiology)} Wounds Found
                  </p>
                  <p className="text-muted-foreground mt-1">
                    No wounds matching this etiology in the selected date range
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge className={cn(getEtiologyColor(etiology))}>
                      ...
                      {patient.wounds.map((wound, idx) => {
                        ...
                        >
                          ...
                          {wound.encounter_history && wound.encounter_history.length > 1 && wound.etiology?.toLowerCase().includes('pressure') && (() => {
                                ...
                                const displayDataPush = [...pushChartData].reverse();
                                ...
                                <div className="mt-4 pt-4 border-t bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-950/30 dark:to-red-950/30 p-3 rounded-md -mx-3">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-[2fr_3fr] gap-4">
                                    ...
                                    <div className="space-y-1 max-h-[180px] overflow-y-auto">
                                      {displayDataPush.map((enc, idx) => {
                                        const isFirst = idx === displayDataPush.length - 1;
                          ...
                          {wound.encounter_history && wound.encounter_history.length > 1 && (() => {
                                ...
                                const chartDisplayDataSurface = [...chartData].reverse();
                                ...
                                <div className="mt-4 pt-4 border-t">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-[2fr_3fr] gap-4">
                                    ...
                                    <div className="space-y-1 max-h-[150px] overflow-y-auto">
                                      {chartDisplayDataSurface.map((enc, idx) => {
                                        const isFirst = idx === chartDisplayDataSurface.length - 1;
                                        const isSelected = enc.date === selectedDateFormatted;
