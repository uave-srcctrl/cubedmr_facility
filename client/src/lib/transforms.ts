/**
 * Data transformation functions for dashboard and report responses.
 * 
 * Moved from server/helpers/transforms.ts to client-side.
...
export function transformToWoundReductionFormat(backendData: any) {
    ...
    // Weekly data → aggregate to monthly
    if (Array.isArray(data) && data.length > 0) {
...
const WOUNDS_BY_STATUS_COLORS: Record<string, string> = {
    ...
    if (Array.isArray(data) && data.length > 0 && (data[0].status || data[0].name)) {
        return {
            ...
            data: data.map((item: any) => ({
                ...
                color: WOUNDS_BY_STATUS_COLORS[item.status || item.name] || '#6b7280',
            })),
            source: "backend",
        };
    }
...
// ─── Etiology metric_name/metric_value transform ────────────────────────────────

/**
 * Transform PHP metric_name/metric_value format into woundEtiology/count format
 * Used by etiology-report.tsx when fetching via apiCall
 */
export function transformEtiologyMetrics(rawData: any[]): any[] {
    if (!Array.isArray(rawData) || rawData.length === 0) return rawData;
    if (rawData[0].metric_name === undefined) return rawData;
    ...
    return Array.from(countMap.entries()).map(([etiology, count]) => ({
