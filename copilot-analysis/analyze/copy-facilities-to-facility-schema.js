...
async function main() {
  ...
  try {
    ...
    const tableResult = await pool.request().query(`
      ...
      FROM sys.objects 
      WHERE object_id = OBJECT_ID(N'[dbo].[facilities]') 
      AND type in (N'U')
    `);
    ...
    try {
      ...
      const result = await pool.request().query(`
        INSERT INTO [dbo].[facilities] (FacilityId, FacilityName, IsActive, CreatedDate, UpdatedDate)
        SELECT 
          f.FacilityId,
          f.FacilityName,
