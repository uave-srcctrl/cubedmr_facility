...
BEGIN TRY
  ...
  INSERT INTO [dbo].[facilities] (id, name)
  SELECT 
    f.FacilityId,
    f.FacilityName
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM [dbo].[facilities] local 
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name = 'IDX_facilities_name' AND object_id = OBJECT_ID(N'[dbo].[facilities]'))
BEGIN
  CREATE INDEX IDX_facilities_name ON [dbo].[facilities]([name])
